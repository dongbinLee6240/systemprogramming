...
int main()
{
    ...
    // "FileMg.txt" 파일을 열어서 getfiletime()의 출력을 저장
    ofstream fileMgOut("FileMg.tax");

    if (!fileMgOut)
    {
        cerr << "FileMg.tax 파일 열기 실패" << endl;
        return 1;
    }
    ...
    ifstream fileMgIn("FileMg.txt");

    if (!fileMgIn)
    {
        cerr << "FileMg.txt 파일 열기 실패" << endl;
        return 1;
    }

    cout << "FileMg.txt 파일의 내용:" << endl;
    cout << fileMgIn.rdbuf(); // 파일 내용 출력
